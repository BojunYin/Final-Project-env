...
# Fig. 6.7.: xy-plot with spatial trend (coast)
library(StatDA)
data(moss)
data(kola.background)

pdf("fig-6-7.pdf",width=9,height=9)
...
X=moss[,"XCOO"]
Y=moss[,"YCOO"]
points(X[X<460000],Y[X<460000],pch=3,cex=0.7)

x=abs((Y[X<460000]-max(Y)+2000)/1000)

y=moss[X<460000,"Cu"]
plot(x,y,xlab="Distance from coast [km]",ylab="Cu in Moss [mg/kg]",pch=3,cex=0.7,cex.lab=1.3)
#lines(smooth.spline(x,y),col=1,lwd=1.3)

y=moss[X<460000,"Ni"]
plot(x,y,xlab="Distance from coast [km]",ylab="Ni in moss [mg/kg]",pch=3,cex=0.7,cex.lab=1.3)
#lines(smooth.spline(x,y),col=1,lwd=1.3)
#dev.new()
dev.off()
